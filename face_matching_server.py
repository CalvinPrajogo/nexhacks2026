...
MATCH_THRESHOLD = 3.0  # Z-score threshold: distances > 3 std deviations are matches
...
def normalize_vector(vec: np.ndarray) -> np.ndarray:
    """Normalize a vector using z-score standardization (more robust than L2)"""
    mean = np.mean(vec)
    std = np.std(vec)
...
def find_matching_person_local(input_features: list, threshold: float = MATCH_THRESHOLD) -> dict:
    ...
    if isinstance(input_features, dict):
        print(f"[Match] Converting features dict to array (dict has {len(input_features)} keys)", flush=True)
        # Extract values in order, skipping metadata fields
        feature_array = []
        ...
        print(f"[Match] Converted to array with {len(input_features)} numeric values", flush=True)
    
    # Handle dimension mismatch
    first_db_face = next(iter(face_database.values()))
    db_feature_count = len(first_db_face['features'])
    input_feature_count = len(input_features)
    
    print(f"[Match] Input features: {input_feature_count}, Database features: {db_feature_count}", flush=True)
    
    # If mismatch, try to pad or truncate
    if input_feature_count != db_feature_count:
        print(f"[Match] WARNING: Feature dimension mismatch! Input={input_feature_count}, DB={db_feature_count}", flush=True)
        if input_feature_count > db_feature_count:
            print(f"[Match] Truncating input features from {input_feature_count} to {db_feature_count}", flush=True)
            input_features = input_features[:db_feature_count]
        else:
            print(f"[Match] Padding input features from {input_feature_count} to {db_feature_count}", flush=True)
            input_features = input_features + [0.0] * (db_feature_count - input_feature_count)
    ...
    for name, data in face_database.items():
        ...
        print(f"[Match] Distance to {name}: {distance:.4f}", flush=True)
        distances.append({
            "name": name,
            "distance": distance,
    ...
    if best_match and best_match['distance'] < threshold:
        ...
        print(f"[Match] MATCH ACCEPTED: {best_match['name']}, confidence: {confidence:.2%}", flush=True)
        return {
            "success": True,
            "matched": True,
        ...
        if best_match:
            print(f"[Match] MATCH REJECTED: {best_match['name']} distance {best_match['distance']:.4f} >= threshold {threshold}", flush=True)
        return {
            "success": True,
            "matched": False,
